...
import { NestingPart, NestingSlab, NestingResult, PlacedPart } from '@/lib/nesting-types';
...
export function useNestingOptimization({ kerfWidth }: UseNestingOptimizationOptions) {
  ...
  const optimize = useCallback(async (parts: NestingPart[], slabs: NestingSlab[]) => {
    if (parts.length === 0) {
